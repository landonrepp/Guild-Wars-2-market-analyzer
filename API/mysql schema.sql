...
--
-- Current Database: `gw2`
--

CREATE DATABASE /*!32312 IF NOT EXISTS*/ `gw2` /*!40100 DEFAULT CHARACTER SET utf8mb4 */;
...
--
-- Dumping routines for database 'gw2'
--
...
DELIMITER ;;
CREATE DEFINER=`landonrepp`@`localhost` PROCEDURE `getDBSize`()
BEGIN
...
/*!50003 SET sql_mode              = 'NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`landonrepp`@`localhost` PROCEDURE `getHotItems`()
BEGIN
...
/*!50003 SET sql_mode              = 'NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`landonrepp`@`localhost` PROCEDURE `getMarketHistory`(item integer)
BEGIN
    SET @line = 0;
    SELECT time, buyQuant, sellQuant,buyPrice, sellPrice from (
        select
            DATE_FORMAT(buy.time,'%Y-%m-%d %H:%i') as time,
            (@line := CASE when @line<20 then @line+1 else 0 end) as lineNumber,
            buy.quantity as buyQuant,
            sell.quantity as sellQuant,
            buy.unitPrice as buyPrice,
            sell.unitPrice as sellPrice
        from tblBuyOrders as buy 
        USE INDEX (buyMarketHist)
        inner join tblSellOrders as sell
        USE INDEX (sellMarketHist)
            on buy.itemsID = sell.itemsID and buy.time = sell.time
        where buy.itemsID = item
    ) sub where lineNumber = 0;
...
/*!50003 SET sql_mode              = 'NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`landonrepp`@`localhost` PROCEDURE `getSidebarLines`()
BEGIN
    select itm.*,
...
/*!50003 SET sql_mode              = 'NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`landonrepp`@`localhost` PROCEDURE `getStoredProcedureList`()
begin
...
/*!50003 SET sql_mode              = 'NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`landonrepp`@`localhost` PROCEDURE `test`()
begin select * from tblItems; end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
/*!50003 SET character_set_results = @saved_cs_results */ ;
...
-- Dump completed on 2019-05-27  4:23:08
